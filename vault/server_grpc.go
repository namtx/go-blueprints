...
func DecodeGPRCValidateRequest(ctx context.Context, r interface{}) (interface{}, error) {
	req := r.(*pb.ValidateRequest)
...
func NewGRPCServer(ctx context.Context, endpoints Endpoints) pb.VaultServer {
	return &grpcServer{
		...
		validate: grpctransport.NewServer(endpoints.ValidateEndpoint, DecodeGPRCValidateRequest, EncodeGRPCValidateResponse),
	}
}
